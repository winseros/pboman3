...
namespace pboman3::test {
    ...
    TEST(ParcelManagerTest, UnpackTree_Unpacks_Parcel) {
        ...
        ASSERT_TRUE(dynamic_cast<FileBasedBinarySource*>(root.child(0)->binarySource.get()));

        const auto* pboBs = dynamic_cast<PboBasedBinarySource*>(root.child(1)->child(0)->binarySource.get());
        ASSERT_TRUE(pboBs);
